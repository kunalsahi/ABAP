CLASS zcl_fm_cs_hier_amdp DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get
        FOR TABLE FUNCTION zi_cshiertf.
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.
...
CLASS zcl_fm_cs_hier_amdp IMPLEMENTATION.

  METHOD get BY DATABASE FUNCTION
     FOR HDB LANGUAGE SQLSCRIPT
     OPTIONS READ-ONLY
     USING setheader setheadert setnode setleaf csks cskt.
   ...
   lt_rows_kostl_parent =
(
  /* --------------------------------------------
     a) SINGLE VALUES: valfrom = valto -> '=' join
     -------------------------------------------- */
  SELECT DISTINCT
    c.mandt                                   AS client,
    :p_fikrs                                  AS fikrs,
  ...
  JOIN csks c
    ON  c.mandt = j.mandt
   AND c.kostl BETWEEN j.valfrom AND j.valto
  LEFT JOIN setheader sh
    ON  sh.mandt    = j.mandt
   AND sh.setclass = lv_setclass
   AND sh.setname  = j.leaf_set
  ...
  WHERE j.valfrom <> j.valto
    ...
    lt_rows_set_to_parent =
      SELECT DISTINCT
        a.mandt                                   AS client,
        :p_fikrs                                  AS fikrs,
        CAST(a.setname AS nvarchar(24))           AS kostl,          -- child (SET name)
        'S'                                       AS nodekind,
  ...
  LEFT JOIN setheadert shta
    ...
    -----------------------------------------------------------------------
    RETURN
      SELECT * FROM :lt_rows_kostl_parent
      UNION ALL
